...
function Swipe(container, options) {
  ...
  var carouselDirection = options.carouselDirection || 'horizontal';
  ...
  function slide(to, slideSpeed) {
    ...
    if (browser.transitions) {
      ...
      if (options.continuous) 
      {
        ...
        direction = -slidePos[circle(to)] / ((carouselDirection == 'horizontal') ? width : height);
  ...
  var events = {
    ...
    handleEvent: function(event) {
      ...
      switch (event.type) {
        ...
        case 'touchend': (carouselDirection == 'horizontal') ? offloadFn(this.endHorizontal(event)) : offloadFn(this.endVertical(event)); break;
    ...
    move: function(event) {
      ...
      if ( typeof isScrolling == 'undefined') {
        isScrolling = !!( isScrolling || ((carouselDirection == 'horizontal' && Math.abs(delta.x) < Math.abs(delta.y)) || (carouselDirection == 'vertical' && Math.abs(delta.x) > Math.abs(delta.y)))  );
      }
          ...
          if (options.continuous) { // we don't add resistance at the end
            ...
            delta.y = 
              delta.y / 
                ...
                ( Math.abs(delta.y) / width + 1 )      // determine resistance level
                : 1 );                                 // no resistance if false
